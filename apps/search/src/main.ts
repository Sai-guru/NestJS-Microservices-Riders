...
async function bootstrap() {
  ...
  const PORT = Number(process.env.SEARCH_TCP_PORT ?? 4002);
...
//creating the 1st microservice instance....
  ...
  //start our rabbitmq-nestjs-microservices container in portainer(or docker desktop) and then run this search service, then you will see the below log in the console
  logger.log(`Search RMQ is listening on queue ${queueName} via ${rmqUrl}`);
